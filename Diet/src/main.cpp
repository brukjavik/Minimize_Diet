...
void solve(Data& data)
{
   ...
   IloFloatVarArray x(env, data.getNItems(), 0, IloInfinity);
   ...
   for(int i = 0; i < data.getNItems(); i++) 
   {
	...
	{
		  std::cout << bpp.getValue(x[i]) << "g do item " << data.getName(i) << std::endl;
	}
   }
}
